    ...
    public function calculerEtoilesDoublees(Compte $compte, Album $album): int
    {
        ...
        foreach ($cas as $ca) {
            ...
            if ($ca->getCarte()->getS()->getAlbum() == $album && $ca->getNombre() > 1) {
                ...
                $etoiles = $ca->getCarte()->getNbetoile();

                // Si la carte est dorée, on double ses étoiles
                if ($ca->getCarte()->isGolden()) {
                    $etoiles *= 2;
                }

                $ajout = $nombre * $etoiles;
                $totE += $ajout;

                $this->logger->info(
                    "Carte: {$ca->getCarte()->getName()} | nb: {$nombre} | étoiles: {$ca->getCarte()->getNbetoile()} | dorée: " . ($ca->getCarte()->isGolden() ? 'oui' : 'non') . " | ajout: {$ajout} | Total: {$totE}"
                );
            }
        }
